...
#### **3. Formulas**
*   **Branch if Equal**: `beq rs1, rs2, label` — branches to `label` if `rs1 == rs2`
*   **Branch if Not Equal**: `bne rs1, rs2, label` — branches to `label` if `rs1 != rs2`
*   **Branch if Less Than**: `blt rs1, rs2, label` — branches to `label` if `rs1 < rs2` (signed)
*   **Branch if Greater or Equal**: `bge rs1, rs2, label` — branches to `label` if `rs1 >= rs2` (signed)
*   **Branch if Less Than Unsigned**: `bltu rs1, rs2, label` — branches to `label` if `rs1 < rs2` (unsigned)
*   **Branch if Greater or Equal Unsigned**: `bgeu rs1, rs2, label` — branches to `label` if `rs1 >= rs2` (unsigned)
*   **Jump and Link**: `jal rd, label` — sets `rd = PC + 4`, then jumps to `label`
*   **Jump and Link Register**: `jalr rd, offset(rs)` — sets `rd = PC + 4`, then jumps to `rs + offset`
*   **Bitwise AND**: `and rd, rs1, rs2` — `rd = rs1 & rs2` (bitwise)
*   **Bitwise OR**: `or rd, rs1, rs2` — `rd = rs1 | rs2` (bitwise)
*   **Bitwise XOR**: `xor rd, rs1, rs2` — `rd = rs1 ^ rs2` (bitwise)
*   **AND Immediate**: `andi rd, rs1, imm` — `rd = rs1 & imm`
*   **OR Immediate**: `ori rd, rs1, imm` — `rd = rs1 | imm`
*   **XOR Immediate**: `xori rd, rs1, imm` — `rd = rs1 ^ imm`
*   **Set Less Than**: `slt rd, rs1, rs2` — `rd = 1` if `rs1 < rs2` (signed), else `rd = 0`
*   **Set Less Than Unsigned**: `sltu rd, rs1, rs2` — `rd = 1` if `rs1 < rs2` (unsigned), else `rd = 0`
*   **Set Less Than Immediate**: `slti rd, rs1, imm` — `rd = 1` if `rs1 < imm` (signed), else `rd = 0`
*   **Shift Left Logical**: `sll rd, rs1, rs2` — `rd = rs1 << rs2` (shifts left by `rs2[4:0]` positions)
*   **Shift Left Logical Immediate**: `slli rd, rs1, shamt` — `rd = rs1 << shamt` (multiply by $2^{shamt}$)
*   **Shift Right Logical**: `srl rd, rs1, rs2` — `rd = rs1 >> rs2` (unsigned, zero-fill)
*   **Shift Right Logical Immediate**: `srli rd, rs1, shamt` — `rd = rs1 >> shamt` (divide by $2^{shamt}$ for unsigned)
*   **Shift Right Arithmetic**: `sra rd, rs1, rs2` — `rd = rs1 >> rs2` (signed, sign-extend)
*   **Shift Right Arithmetic Immediate**: `srai rd, rs1, shamt` — `rd = rs1 >> shamt` (signed division by $2^{shamt}$)
*   **Shift Left by** $n$ **positions**: Equivalent to multiplying by $2^n$
*   **Shift Right by** $n$ **positions**: Equivalent to dividing by $2^n$ (for unsigned or using arithmetic shift for signed)

#### **4. Examples**
##### **4.1. Array Filling with Constant Value** (Lab 12, Example 1)
...
##### **4.2. Array Filling with Indices** (Lab 12, Discussion Question)
...
##### **4.3. Factorial Calculation** (Lab 12, Task 1)
...
##### **4.4. Word Counting in a String** (Lab 12, Task 2)
...
##### **4.5. Binary Search Implementation** (Lab 12, Optional Task)
...
##### **4.6. Logical Expression Implementation** (Lecture 12, Example)
...
##### **4.7. String Reading and Printing** (Lecture 12, Example)
...
##### **4.8. Fast Multiplication Using Shift** (Lecture 12, Example)
...
##### **4.9. Fast Division Using Shift** (Lecture 12, Example)
...
##### **4.10. Infinite Loop with Jump** (Lecture 12, Example)
...
##### **4.11. Function Call with JAL/JALR** (Lecture 12, Example)

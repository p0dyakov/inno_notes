---
title: "W11. RISC-V Instruction Set Architecture, Assembly Programming, Program Translation"
...
- **Fetches instructions** from memory using the address stored in the PC
![](9_10.png){width=80%}
- **Decodes instructions** to determine which operation to perform and which registers to use
- **Controls the execution flow** by directing operations to the ALU or other units
- **Communicates with I/O devices** to handle input/output operations
- **Executes control flow instructions** like branches (`beq`, `bne`) and jumps (`jal`, `jalr`) that change the program's execution order

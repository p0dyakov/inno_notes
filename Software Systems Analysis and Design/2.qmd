...
*   **Static Declaration Semantics**: Allocate memory on stack → Evaluate initializer → Convert to target type if needed → Store value in object
*   **Dynamic Declaration Semantics**: Allocate memory on heap with `new` → Call constructor → Return pointer to object; Cleanup requires `delete` → Call destructor → Free memory
*   **Member Access via Object**: `object.member`
*   **Member Access via Pointer**: `pointer->member`
*   **Object Lifetime (Stack)**: From declaration to end of enclosing scope `{}`
*   **Object Lifetime (Heap)**: From `new` operator to `delete` operator
*   **Copy Constructor Signature**: `ClassName(const ClassName& other)`
*   **Operator Function Signature**: `ReturnType operator@(parameters)` where `@` is the operator symbol
*   **Conversion Function Signature**: `operator TargetType() const`

#### **4. Examples**
##### **4.1. Create a Box Class with Basic Constructors** (Lab 2, Task 1)

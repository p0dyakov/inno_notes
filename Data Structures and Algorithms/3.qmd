...
**Output:** Indices $l$ and $r$ such that $\sum_{i=l}^{r} a_i \ge \sum_{i=l'}^{r'} a_i$ for any $1 \le l' \le r' \le n$.

In other words, find the contiguous subarray with the maximum sum.
...
##### **4.4. Trace Merge Sort Example** (Lecture 3, Example)
...
##### **4.5. Trace Quicksort Partition Example** (Lecture 3, Example)
...
##### **4.6. Find Maximum Subarray Using Divide and Conquer** (Lecture 3, Example)
...
##### **4.7. Analyze Merge Sort Time Complexity** (Lecture 3, Example)
...
##### **4.8. Analyze Quicksort Worst-Case Time Complexity** (Lecture 3, Example)
...
##### **4.10. Yet Another Land of Mountains (Divide and Conquer)** (Homework, Coding Exercise A)

You have an $N \times M$ grid where each cell contains a mountain height. A cell is **safe** if its height is strictly less than all of its neighbors (up, down, left, right). After $Q$ update queries that modify mountain heights, find any safe cell using divide and conquer.

**Input:** Grid dimensions $N$, $M$; number of queries $Q$; initial grid of heights; $Q$ update queries.

**Output:** One safe cell (row and column) after all updates.

<details>
<summary>Click to see the solution</summary>

**Key Concept:** Use divide and conquer on the 2D grid. If a cell is safe in a subregion, we found our answer. If not, eliminate that subregion and search another.

1.  **Algorithm idea:**
    - Divide the grid into four quadrants
    - Check if the center (intersecting point) is safe
    - If yes, return it
    - If no, recursively search one of the quadrants that must contain a safe cell

    Why must a quadrant contain a safe cell? Because a local minimum must exist in any finite region. We can prune quadrants strategically based on which neighbors of the center cell have smaller values.

2.  **Key insight for pruning:**
    - If the center is not safe, at least one neighbor is smaller
    - Search the quadrant toward that smaller neighbor
    - By invariant, that quadrant must contain a local minimum

3.  **Pseudocode sketch:**
    ```
    function find_safe(grid, top, bottom, left, right):
      if (grid is small):
        check all cells, return any safe cell

      row_mid = (top + bottom) / 2
      col_mid = (left + right) / 2

      center = grid[row_mid][col_mid]

      // Check four neighbors
      neighbors = [UP, DOWN, LEFT, RIGHT] = [
        grid[row_mid - 1][col_mid],  // up
        grid[row_mid + 1][col_mid],  // down
        grid[row_mid][col_mid - 1],  // left
        grid[row_mid][col_mid + 1]   // right
      ]

      if all neighbors > center:
        return (row_mid, col_mid)  // center is safe!
      else:
        // Find which neighbor is smaller
        smallest_direction = argmin(neighbors)
        // Search the quadrant in that direction
        return find_safe(grid, ..., smallest_direction, ...)
    ```

4.  **Time complexity:**
    - Best case: $O(1)$ if center is safe
    - Worst case: $O(N \cdot M)$ if we search all quadrants
    - Average: Recursion tree depth is $O(\log(N \cdot M))$ with constant branching
    - Overall: $O(N \cdot M)$ because we may need to check many cells

5.  **Why divide and conquer is used:**
    - Problem guarantees that a safe cell exists
    - Divide and conquer with pruning avoids brute-force checking all cells
    - Each recursive step either finds the answer or eliminates a quadrant

**Answer:** Use divide and conquer with recursive quadrant search, pruning based on which neighbor of center is smallest.

</details>

##### **4.11. Nearest Restaurant (Voronoi Diagram via Divide and Conquer)** (Homework, Coding Exercise B)

You have a grid with $K$ restaurants at known locations. For each of $P$ people asking "which restaurant is closest?", determine the nearest restaurant using a divide-and-conquer Voronoi algorithm. **Euclidean distance**: $\sqrt{(X_i - X_j)^2 + (Y_i - Y_j)^2}$.

<details>
<summary>Click to see the solution</summary>

**Key Concept:** Build a Voronoi diagram using divide and conquer. The Voronoi diagram partitions the grid into regions, where each region contains all points closest to a particular restaurant.

1.  **Voronoi diagram via divide and conquer:**
    - A Voronoi region for restaurant $R$ is the set of all points closer to $R$ than to any other restaurant.
    - Two approaches:
      a. Build the full Voronoi diagram, then answer queries
      b. For each query, use divide and conquer to find the nearest restaurant directly

2.  **Direct approach (divide and conquer per query):**
    ```
    function nearest_restaurant(restaurants, query_point, left, right):
      if only one restaurant in range:
        return that restaurant

      mid = (left + right) / 2
      left_restaurants = restaurants[left..mid]
      right_restaurants = restaurants[mid+1..right]

      left_nearest = nearest_restaurant(left_restaurants, query_point)
      right_nearest = nearest_restaurant(right_restaurants, query_point)

      dist_left = distance(query_point, left_nearest)
      dist_right = distance(query_point, right_nearest)

      nearest = (dist_left < dist_right) ? left_nearest : right_nearest
      return nearest
    ```

3.  **Complexity per query:**
    - $T(n) = 2T(n/2) + O(1) = O(n)$ to find nearest among all $n$ restaurants
    - With $P$ queries: $O(P \cdot n)$

4.  **Optimized Voronoi approach (build once, query fast):**
    - Build the Voronoi diagram by recursively partitioning the restaurant list
    - For each partition, compute the separating line between left and right restaurants
    - Mark grid cells by which restaurant is nearest
    - Queries become $O(1)$ after $O(N \cdot M)$ preprocessing

5.  **Handling ties:**
    - If two restaurants are equidistant, choose the **lexicographically smallest name** (alphabetically first)
    - This requires comparing names when distances are equal

6.  **Pseudocode for preprocessing (simplified):**
    ```
    function build_voronoi(restaurants, grid_cells):
      if only one restaurant:
        mark all cells as nearest to that restaurant
      else:
        split restaurants list in half
        build_voronoi(left_restaurants, left_cells)
        build_voronoi(right_restaurants, right_cells)

        for each cell in shared boundary:
          compute distance to nearest restaurant on left
          compute distance to nearest restaurant on right
          mark cell with the closer restaurant (or lexicographically smaller if tied)
    ```

**Answer:**
- **Approach 1 (Direct):** For each query, perform divide-and-conquer search among all restaurants. Complexity: $O(P \cdot K)$.
- **Approach 2 (Voronoi):** Preprocess to build Voronoi diagram using divide and conquer. Complexity: $O(N \cdot M \cdot K)$ preprocessing, $O(1)$ per query.
...
2.  **Why Master Theorem fails:**
    ...
    - For the recurrence to make sense, we need decreasing subproblem sizes, but $4n/9 > n$ for the recurrence to eventually reach a base case... wait, actually $4/9 < 1$, so $4n/9 < n$. Let me reconsider.

    Actually, $4n/9 = (4/9)n \approx 0.444n$, so subproblems DO shrink. The issue is that we have 5 subproblems of size $4n/9$, not of size $n/b$.

    We can view this as: $T(n) = 5T(n \cdot 4/9) + n^2$. Rewrite as: Let $n' = n$, then $T(n') = 5T(n' \cdot 4/9) + n'^2$.

    This is still not in the standard form $T(n) = aT(n/b) + f(n)$. The Master Theorem, as stated in Cormen et al., requires the subproblems to be of size exactly $n/b$.

...
#### **4. Examples**
##### **4.1. Prove Lower Bound for Comparison-Based Sorting** (Lecture 4, Example)
...
##### **4.2. Apply Radix Sort to Decimal Numbers** (Lecture 4, Example)
...
##### **4.3. Analyze Bucket Sort Average Case** (Lecture 4, Example)
...
##### **4.4. Decision Tree for Sorting Three Elements** (Lecture 4, Example)
...
```
      ...
      a≤b≤c      a≤c<b   b≤a≤c    b≤c<a
                        ...
                        YES      NO
                        /         \
                     b≤a≤c      c<a≤b  (Not reachable, contradiction)
```
...
```
        ...
        (Leaf 1)  (Leaf 2) (Leaf 3) (Leaf 4)
                             ...
                             b≤c?  b≤c?
                            /   \  /   \
                          YES   NO (not reachable)
                          /      \
                       b≤c<a   c<b<a
                      (Leaf 5) (Leaf 6)
```

Wait, let me simplify. The six leaves correspond to the six permutations of three elements:
...
##### **4.5. Stability of Radix Sort** (Lecture 4, Example)
...
##### **4.6. Compare Sorting Algorithms on Different Data** (Lecture 4, Exercise)
...
##### **4.7. Estimate Complexity of Radix Sort with Arbitrary Radix** (Lecture 4, Exercise)
...
##### **4.8. Time Complexity of Radix Sort with Variable-Radix** (Lecture 4, Exercise)
...
##### **4.9. Range Queries Using Counting Sort** (Lecture 4, Exercise)
...
##### **4.10. Apply Counting Sort** (Problem Set 4, Problem 1)
...
##### **4.11. Radix Sort Complexity for Programs** (Problem Set 4, Problem 2)
...
##### **4.12. Decision Tree for Quicksort Variation** (Problem Set 4, Problem 3)
...
##### **4.13. The Magical Donut** (Homework, Coding Exercise A)

Natasha prepared a big donut where the inner radius is $R_1$ and the outer radius is $R_2$ (concentric circles, origin at center). She throws $N$ frosting pieces that fall uniformly at random on the annulus surface.

Given the Cartesian coordinates of the frostings, **sort them by distance from the center** (nearest to farthest). If two frostings have the same distance, sort by $x$-coordinate, then by $y$-coordinate.

**Input:**
- First line: three integers $N$ ($1 \le N \le 10^5$), $R_1$, $R_2$ ($1 \le R_1 < R_2 \le 10^9$)
- Next $N$ lines: $X_i$, $Y_i$ where $R_1^2 \le X_i^2 + Y_i^2 \le R_2^2$ (all points unique, inside annulus)

**Output:** Print $N$ points after sorting, one per line in format `X Y`
...
**Key Concept:** Compute squared distance for each point, then sort using a stable comparison with distance as primary key.

1. **Compute squared distance:** $d_i^2 = X_i^2 + Y_i^2$ for each frosting $i$
   - Using squared distance avoids floating-point errors
   - Preserves ordering of distances

2. **Create sorting key:** For each point $(X, Y)$, create triple $(d^2, X, Y)$ where $d^2$ is squared distance
   - Primary sort: by $d^2$ (distance)
   - Secondary sort: by $X$ (x-coordinate)
   - Tertiary sort: by $Y$ (y-coordinate)

3. **Sort points** using the triple as comparison key

4. **Example walkthrough:**

   | Point | $X^2 + Y^2$ | Distance | Sorted by $(d^2, X, Y)$ |
   | --- | --- | --- | --- |
   | $(0, -6)$ | $0 + 36 = 36$ | $6$ | $(36, 0, -6)$ |
   | $(5, 0)$ | $25 + 0 = 25$ | $5$ | $(25, 5, 0)$ |
   | $(-7, 0)$ | $49 + 0 = 49$ | $7$ | $(49, -7, 0)$ |
   | $(0, 5)$ | $0 + 25 = 25$ | $5$ | $(25, 0, 5)$ |
...
```python
points = read_input()
sorted_points = sort(points, key=lambda p: (p.x**2 + p.y**2, p.x, p.y))
for point in sorted_points:
    print(point.x, point.y)
```

6. **Complexity:** $\Theta(n \log n)$ for sorting (using built-in sort)

**Answer:** Compute squared distance for each point, sort using triple $(d^2, X, Y)$ as key, and output sorted points.

</details>

##### **4.14. Cormen-Adabra** (Homework, Coding Exercise B)

A wizard's spellbook contains $N$ distinct words. A spell is a sequence of indices referring to words from the spellbook. Spells are stored in arbitrary order. Your task is to **sort spells lexicographically** based on the concatenated strings of the words they reference.

**Input:**
- First line: $N$ (words) and $M$ (spells), where $1 \le N, M \le 10^4$
- Second line: $N$ distinct words separated by spaces (each word: 1-20 lowercase letters)
- Next $M$ lines: Spell definition
  - Integer $K$ ($1 \le K \le 10$) — size of spell
  - $K$ integers (1-indexed) representing word indices in the spell

**Output:** Print $M$ spells in lexicographical order (concatenated form), using same format as input

**Example:**
```
Input:
15 3
doing work great the are good things keep you and come up from perseverance hard
4 9 5 1 3
5 8 12 4 6 2
8 3 7 11 13 15 2 10 14
...
**Explanation of spells (with word concatenation):**
- Spell 1: indices $[9, 5, 1, 3]$ → words $[\text{you}, \text{are}, \text{doing}, \text{great}]$ → `youaredoinggreat`
- Spell 2: indices $[8, 12, 4, 6, 2]$ → words $[\text{keep}, \text{up}, \text{the}, \text{good}, \text{work}]$ → `keepupthegoodwork`
- Spell 3: indices $[3, 7, 11, 13, 15, 2, 10, 14]$ → words $[\text{great}, \text{things}, \text{come}, \text{from}, \text{hard}, \text{work}, \text{and}, \text{perseverance}]$ → `greatthingscomefromhardworkandperseverance`

Sorted lexicographically: `greatthingscomefromhardworkandperseverance` < `keepupthegoodwork` < `youaredoinggreat`

<details>
<summary>Click to see the solution</summary>

**Key Concept:** Concatenate words for each spell, then sort spells by the resulting strings lexicographically.

1. **Read input:**
   - Read $N$ words into array `words[]`
   - For each of $M$ spells, read the indices

2. **For each spell:**
   - Concatenate the words referenced by its indices into a single string
   - Store both the concatenated string and the original spell (indices)

3. **Sort spells** by concatenated string lexicographically

4. **Output** the sorted spells in original format (indices)

5. **Algorithm (pseudocode):**
```python
N, M = read_integers()
words = read_words()  # words[0] is word 1, etc.

spells = []
for i in range(M):
    K = read_integer()
    indices = read_integers()  # 1-indexed

    # Concatenate words
    concatenated = ""
    for idx in indices:
        concatenated += words[idx - 1]  # Convert to 0-indexed

    # Store (concatenated string, original indices)
    spells.append((concatenated, indices))

# Sort by concatenated string
spells.sort(key=lambda x: x[0])

# Output sorted spells
for concatenated, indices in spells:
    print(len(indices), " ".join(map(str, indices)))
```

6. **Example execution:**

   Words: `doing work great the are good things keep you and come up from perseverance hard`

   Spell 1: `[9, 5, 1, 3]` → `you` + `are` + `doing` + `great` = `youaredoinggreat`

   Spell 2: `[8, 12, 4, 6, 2]` → `keep` + `up` + `the` + `good` + `work` = `keepupthegoodwork`

   Spell 3: `[3, 7, 11, 13, 15, 2, 10, 14]` → `great` + `things` + `come` + `from` + `hard` + `work` + `and` + `perseverance` = `greatthingscomefromhardworkandperseverance`

   Sorted lexicographically:
   1. `greatthingscomefromhardworkandperseverance` (Spell 3)
   2. `keepupthegoodwork` (Spell 2)
   3. `youaredoinggreat` (Spell 1)
...
7. **Complexity:**
   - Concatenation: $O(M \cdot K \cdot L)$ where $L$ is average word length (max $M \cdot 10 \cdot 20 = 2 \times 10^5$)
   - Sorting: $O(M \log M)$ comparisons of strings (each comparison $O(K \cdot L) = O(200)$)
   - Total: $O(M \log M \cdot K \cdot L) = O(10^4 \cdot \log(10^4) \cdot 10 \cdot 20) \approx O(3 \times 10^7)$ — acceptable

**Answer:** Read all spells, concatenate their words, sort lexicographically by concatenated strings, and output sorted spells in original (index) format.</details>

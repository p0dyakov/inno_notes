...
#### **4. Examples**
##### **4.1. Compute Asymptotic Worst-Case Time Complexity** (Problem Set 1, Problem 1.1)
...
##### **4.2. Loop Invariant Proof for Reorder Algorithm** (Problem Set 1, Problem 1.2)
...
##### **4.3. Asymptotic Notation: True or False** (Problem Set 1, Problem 1.3)
...
##### **4.11. Prove Correctness and Analyze exp1 Algorithm** (Lab 1, Exercise 1.1)
...
##### **4.12. Prove Correctness and Analyze exp2 Algorithm** (Lab 1, Exercise 1.2)
...
##### **4.13. GCD Algorithm (Naive)** (Lab 1, Exercise 1.3)
...
##### **4.14. Euclidean GCD Algorithm** (Lab 1, Exercise 1.4)
...
##### **4.15. Algorithm Design: Pi Approximation** (Homework, Coding Exercise A)

Calculate an approximation of the number $\pi$ using the Leibniz formula:

$$\frac{\pi}{4} = 1 - \frac{1}{3} + \frac{1}{5} - \frac{1}{7} + \frac{1}{9} - \dots$$

**Input:** One number $N$ ($0 \le N \le 10^6$) — the number of iterations.

**Output:** One number — the approximation of $\pi$ with six decimal places.

<details>
<summary>Click to see the solution</summary>

**Key Concept:** Sum the first $N$ terms of the Leibniz series and multiply by 4 to get $\pi$.
...
function pi_approximation(N):
    sum = 0.0
    for i = 0 to N - 1:
        sign = (-1) ^ i                # alternates: +1, -1, +1, -1, ...
        denominator = 2 * i + 1       # 1, 3, 5, 7, 9, ...
        term = sign / denominator
        sum = sum + term
    pi_approx = 4 * sum
    return round(pi_approx, 6)        # round to 6 decimal places
```

2.  **Why it works:** The Leibniz formula expresses $\pi/4$ as an infinite series. The more terms we sum, the more accurate our approximation becomes.

3.  **Time Complexity:** $O(N)$ — we iterate $N$ times and perform $O(1)$ arithmetic per iteration.

4.  **Space Complexity:** $O(1)$ — only a constant amount of extra memory for variables.

**Answer:** Use the Leibniz formula with $N$ iterations. Time: $O(N)$, Space: $O(1)$.

</details>

##### **4.16. Algorithm Design: Nearly Ordered Sequence** (Homework, Coding Exercise B)

You are given a sequence of numbers which is already nearly ordered. Can you sort it quickly?

Implement a sorting algorithm as a separate function. The sorting function must accept an array as an argument and either return a new sorted array or sort the input array in place.

**Input:** First line contains an integer $N$ ($0 \le N \le 10^6$). Second line contains $N$ integers ($a_1, a_2, \dots, a_N$, where $-10^9 \le a_i \le 10^9$), separated by single spaces.

**Output:** An ordered sequence of numbers separated by spaces.

<details>
<summary>Click to see the solution</summary>

**Key Concept:** A "nearly ordered" sequence has most elements close to their sorted position. Use **Insertion Sort**, which is efficient when the array is already nearly sorted.

1.  **Why Insertion Sort:** For a nearly sorted array, most elements require only a few comparisons and shifts. If each element is at most $k$ positions from its final position, Insertion Sort runs in $O(nk)$ time rather than $O(n^2)$.
...
3.  **Explanation:**
    - For each element starting from index 2, compare it with previous elements (working backward).
    - Shift larger elements one position to the right.
    - Insert the current element into its correct position.

4.  **Time Complexity:** For a nearly sorted array where elements are within distance $k$ of their final position, $T(n) = O(nk)$. If $k$ is small (say, constant), this is $O(n)$ — linear time! Even in the worst case (completely reversed array), $T(n) = O(n^2)$.

5.  **Space Complexity:** $O(1)$ — in-place sorting with no auxiliary data structures.

**Answer:** Use Insertion Sort. For nearly sorted arrays with small displacement: $O(n)$ time. Worst case: $O(n^2)$ time, $O(1)$ space.

...
##### **4.1. Expected Score in a Coin Game** (Lecture 7, Exercise 1)
...
##### **4.2. Determining Uniform Distributions** (Lecture 7, Example 1)
...
##### **4.3. Dependent Random Variables** (Lecture 7, Example 2)
...
##### **4.4. Expected Number of Heads** (Lecture 7, Example 3)
...
##### **4.5. Probabilistic Analysis of the Hiring Problem** (Lecture 7, Example 4)
...
##### **4.6. Expected Height of a Randomly Chosen BST** (Lecture 7, Exercise 2)
...
##### **4.7. Expected Height of a Randomly Built BST** (Lecture 7, Exercise 3)
...
##### **4.8. Proving or Disproving Probability Identities** (Problem Set 7, Problem 1)
...
##### **4.9. Proving or Disproving Expectation Identities** (Problem Set 7, Problem 2)
...
##### **4.10. Modular-Search Algorithm** (Problem Set 7, Problem 3)
...
##### **4.11. Spectacular Photo (Randomized Line Finding)** (Coding Exercises 7, Problem A)

Astronomers observed $N$ shooting stars. At least $\frac{N}{4}$ of them lie along a single secret "cosmic path" (a straight line). Find the line through which the maximum number of stars pass and output those stars' indices.

Given $N$ ($4 \leq N \leq 10^6$) and coordinates $(x_i, y_i)$ with $|x_i|, |y_i| \leq 10^9$.

<details>
<summary>Click to see the solution</summary>

**Key Concept:** Since at least $\frac{N}{4}$ stars lie on the answer line, picking two random stars both from the answer line has probability at least $\frac{1}{16}$. Repeat enough times, then verify each candidate line.

1.  **Randomized approach:**
    *   Repeat $O(\log(1/\delta))$ times (for desired failure probability $\delta$):
        *   Pick two stars uniformly at random.
        *   The line through them is a candidate.
        *   Count how many of the $N$ stars lie exactly on this line.
        *   Track the best line found so far.
2.  **Probability analysis:**
    *   In each trial, the probability that both chosen stars lie on the answer line is at least $\frac{N/4}{N} \cdot \frac{N/4 - 1}{N-1} \geq \frac{1}{16}$ (for $N \geq 4$).
    *   After $k$ trials, the probability of never hitting the answer line is at most $\left(\frac{15}{16}\right)^k$.
    *   For $k = 200$ trials, this is $< 10^{-6}$.
3.  **Verifying a candidate line:** Given two points $(x_1, y_1)$ and $(x_2, y_2)$, a point $(x, y)$ lies on the line through them iff:
    $$(y_2 - y_1)(x - x_1) = (x_2 - x_1)(y - y_1)$$
    Use integer arithmetic (no floating point) to avoid precision errors. This check takes $O(N)$ per candidate.
4.  **Total running time:** $O(N \log(1/\delta))$ expected time with failure probability $\delta$.

**Answer:** Use the randomized algorithm: pick random pairs of stars, check if the line through them contains $\geq N/4$ stars, repeat $O(\log N)$ times. The answer line is found with high probability.

</details>

##### **4.12. Almost Unbiased (BST with Random Keys)** (Coding Exercises 7, Problem B)

A university maintains a student database implemented as a BST ordered by student ID. Initial records may be inserted in any order you choose. The IDs are guaranteed to be randomly generated. Implement `insert`, `remove`, and `search` operations *without* self-balancing rotations, and achieve efficient expected search time.

<details>
<summary>Click to see the solution</summary>

**Key Concept:** Since IDs are *randomly generated* (uniformly random integers), a BST built by inserting them in any order is a randomly built BST. By the theory from this lecture, a randomly built BST has expected height $\Theta(\log n)$, so search, insert, and remove all take $O(\log n)$ expected time without any balancing.

**Strategy for initial insertion order:** Insert the $N$ initial records in a *shuffled* (random) order. Since IDs are already random, inserting in any order gives a randomly built BST. However, to be safe (and to ensure the comment matches the theory), shuffling the initial records is a sound strategy.

**Comment to include in source code:**
```
// Strategy: Insert initial records in a randomly shuffled order.
// Since IDs are randomly generated, this produces a randomly built BST.
// Expected height is O(log n), giving O(log n) expected search time
// without any rotations or balancing.
```
...
class BST:
    ...
    def _insert(self, node, record):
        if node is None: return Node(record)
        if record.id < node.id: node.left = self._insert(node.left, record)
        elif record.id > node.id: node.right = self._insert(node.right, record)
        return node
    ...
    def _remove(self, node, id):
        ...
        if id < node.id: node.left = self._remove(node.left, id)
        elif id > node.id: node.right = self._remove(node.right, id)
        else:  # found; replace with in-order successor
            if node.left is None: return node.right
            if node.right is None: return node.left
            successor = min(node.right)
            node.id, node.data = successor.id, successor.data
            node.right = self._remove(node.right, successor.id)
        return node
    ...
    def search(self, id):
        ...
        while node:
            ...
            elif id < node.id: node = node.left
            else: node = node.right
        return NOT_FOUND
```

**Answer:** Use a standard BST with the initial records shuffled. Because IDs are random, the BST has expected height $\Theta(\log n)$, giving efficient expected $O(\log n)$ operations without any self-balancing.

</details>

##### **4.13. Lord of the Matrices (Randomized Matrix Verification)** (Coding Exercises 7, Problem C)

Given three $N \times N$ matrices $A$, $B$, and $C$, determine whether $A \times B = C$ in $O(N^2)$ time.

<details>
<summary>Click to see the solution</summary>

**Key Concept:** Naively computing $A \times B$ takes $O(N^3)$. Instead, use **Freivalds' randomized algorithm**: pick a random vector $r$, compute $A(Br)$ and $Cr$, and check if they are equal. Each matrix-vector multiplication takes $O(N^2)$.

**Algorithm (Freivalds' Algorithm):**

1.  Pick a random vector $r \in \{0, 1\}^N$ (each component independently 0 or 1 with equal probability).
2.  Compute $x = Br$ — matrix-vector product, $O(N^2)$.
3.  Compute $y = Ax$ — matrix-vector product, $O(N^2)$.
4.  Compute $z = Cr$ — matrix-vector product, $O(N^2)$.
5.  If $y = z$, output "Yes"; otherwise output "No".

**Correctness:**

*   If $AB = C$: then $ABr = Cr$ for *any* $r$, so we always output "Yes". (No false negatives.)
*   If $AB \neq C$: let $D = AB - C \neq 0$. We need $Dr = 0$ to get a false "Yes". The probability that $Dr = 0$ for a uniformly random $r \in \{0,1\}^N$ is at most $\frac{1}{2}$ (this follows from the Schwartz-Zippel lemma or a direct analysis).

**Repeating $k$ times** reduces the error probability to $\left(\frac{1}{2}\right)^k$. For $k = 20$, the error probability is $< 10^{-6}$.

**Total time:** $O(k \cdot N^2) = O(N^2)$ for constant $k$.

```python
def freivalds(A, B, C, N, k=20):
    for _ in range(k):
        r = [random.randint(0, 1) for _ in range(N)]
        Br = matrix_vector_mult(B, r, N)   # O(N^2)
        ABr = matrix_vector_mult(A, Br, N) # O(N^2)
        Cr = matrix_vector_mult(C, r, N)   # O(N^2)
        if ABr != Cr:
            return "No"
    return "Yes"
```

**Answer:** Use Freivalds' randomized algorithm with $O(\log(1/\delta))$ repetitions for failure probability $\delta$. Total time: $O(N^2)$.

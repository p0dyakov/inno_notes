...
2. `python3 scripts/agent/generate.py --inno-files /tmp/inno_files --sha …`:
   ...
   - статьи пишутся staged-флоу (цель: self-study с нуля, без воды, без потерь тем).
     ...
     детерминированные гейты без LLM: `fix_formatting.py` ловит висячие
     `equation (N)`-ссылки без `\tag{N}` и дубли слов (advisory, non-blocking),
     `scripts/agent/validate_practice_sources.py` требует, чтобы source-лейблы
     практики резолвились в реальные транскрипты недели (однозначные
     перепутанные виды чинятся сами в `generate.py`, остатки — в карантин;
     плюс строгий шаг в `agent.yml` до коммита);
...
| Быстро посмотреть правки (dev-flow, без деплоя) | `python3 scripts/render_changed.py --base main` + `bash scripts/serve.sh 4210` | Инкрементальный рендер только изменённого vs main (секунды–минуты), затем статика `_site/` на http://localhost:4210 — математика уже бейкнута, навигация мгновенная |
| Рабочая ветка | `git checkout dev`, коммиты + `git push origin dev` | Пуш в dev запускает только быстрый CI-гейт (`check`: YAML + форматирование, без рендера и публикации). В main agente/человек не пушат напрямую |
| В прод (только по запросу) | merge `dev` → `main` | Мерж запускает стандартный `deploy-site` (build + gh-pages). После мержа — хук + сверка прода контентом, как обычно |
...
В dev-ветке base по умолчанию — `main` (`--base main`): рендерится ровно то,
что уйдёт в прод при мерже. CI на `main` использует тот же скрипт.
...
- на время параллельных рендеров project pre/post-render (`update_*`, `bake-static-html.sh`)
  ...
  восстанавливается; bake запускается точечно (`bake-static-html.mjs <файлы>`). bake заодно экстернализует mermaid-SVG в <img> (`<stem>_files/diagram-*.svg`, с alt и purge сирот): инлайн-SVG ломается браузерными forced-dark дорисовками (светлые заливки + белые подписи), а картинка целиком — атомарна и читаема в любом режиме.
...
9. **Деплой без подтверждения = незавершённая работа.** Обычные правки пушатся
   в `dev` (там только быстрый `check`, деплоя нет). В прод уходят мержем
   `dev` → `main` и только по запросу пользователя. Агент, который сделал мерж,
   обязан: взвести хук на resulting `deploy-site` ран (имена логов
   `/tmp/inno_*_hook.log`), дождаться `HOOK_EXIT`, и только потом завершаться.
   - `HOOK_EXIT=0` — сверить ПРОД контентом (`curl` страницы + `grep`), а не
     только зелёным статусом: билд может быть зелёным, а страница старой
     (кеш Pages/SPA/CDN). Не сошлось — разбираться, а не отчитываться об успехе.
   - `HOOK_EXIT=1` — чинить с упавшей стадии по правилу rerun (см. раздел 1),
     предыдущие зелёные стадии НЕ повторяются; после фикса — новый хук.
   - Запрещено завершать итерацию словами "деплой запущен, проверьте сами",
     если только пользователь явно не попросил fire-and-forget.
10. **Статьи из сэмпл-тестов скрываются, а не публикуются.** Если по неделе есть
    только sample-тест (нет lec/tut), статья пишется как обычно (решения из сэмпла
    сохраняются), но прячется: `draft: true` во front matter + убрать из сайдбара
    в `_quarto.yml` + `<!-- HANDWRITTEN -->` от перегенерации. `update_sidebar()`

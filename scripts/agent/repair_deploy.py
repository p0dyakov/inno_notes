...
def ask_gemini_for_patch(log: str, api_key: str) -> str:
    ...
    prompt = (
        ...
        "Deploy (.github/workflows/deploy.yml) has two jobs: build on windows-latest (R 4.5 + P3M snapshot 2025-08-01 pinning knitr 1.50 + rmarkdown 2.29 as binaries, then quarto render, then commit _site/ back) and deploy on ubuntu (publishes _site/ to gh-pages, no toolchain). Never suggest Linux-only steps (apt-get) or unpinned latest R packages. "

...
- ASCII-art diagrams are banned: a fenced block whose lines are mostly
  ...
  Redraw as mermaid (flows) or a matplotlib PNG (plots) per exemplars.
...
- TikZ figures are banned: any ` ```{tikz} ` block is a gate violation.
  Every figure must be a committed matplotlib PNG in the article folder's
  `fig-mpl/` directory (generated by `scripts/figs/`, computed geometry),
  referenced as `![caption](fig-mpl/name_mpl.png){#fig-anchor}`.
- No eyeballed vectors: every arrow MUST start/end at a named, computed
  point (curve point, projection, tangency, vertex) — never at hand-picked
  coordinates. Lengths derive from the figure's own quantities (proportions
  of the drawn objects), directions from the physics (tangent, normal,
  wall/floor axes). A vector floating near its object instead of anchored
  to it is a defect. The gate checks every `fig-mpl/*.png` is produced by
  a `scripts/figs/gen_*.py` script.
...
- ATX headings start their own line with a blank line before (auto-fixed);
  a `$$` paragraph must pair within itself (unpaired `$$` is a gate
  violation). Figures stay readable: horizontal

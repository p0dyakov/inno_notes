...
2.  **Alternative approach (substitution method):**
    - Let $m = \log_3 n$, so $n = 3^m$ and $n^{1/3} = 3^{m/3}$
    - Set $S(m) = T(3^m)$: then $T(3^m) = 3T(3^{m/3}) + 3^m$ becomes $S(m) = 3S(m/3) + 3^m$
    - This has the form $S(m) = aS(m/b) + f(m)$ with $a = 3$, $b = 3$, $f(m) = 3^m$
    - Here $m^{\log_b a} = m^{\log_3 3} = m$, but $f(m) = 3^m$ is exponential in $m$, not polynomial — the Master Theorem does not apply in the $m$ domain either
3.  **Conclusion:**

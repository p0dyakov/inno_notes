...
digraph chomsky {
  ...
  reg [label="Regular Languages\n(Finite State Automata)", fillcolor="#d6eef5", width=2.2, height=0.7];
...
digraph turnstile {
  ...
  Locked   [label="Locked"];
  Unlocked [label="Unlocked", shape=doublecircle, fillcolor="#d7f0c2"];
...
digraph turing_test {
  ...
  screen [label="Partition\n(text only)", shape=diamond, fillcolor="#fff3cd"];
...
digraph vna {
  ...
  cpu [label="Central Processing Unit\n(Control Unit + ALU)", shape=box, fillcolor="#d6eef5", width=3.2];
  mem [label="Memory Unit\n(programs + data)", shape=box, fillcolor="#fff3cd", width=3.2];
  inp [label="Input Device", shape=box, fillcolor="#e8f4f8"];
  out [label="Output Device", shape=box, fillcolor="#d7f0c2"];
...
digraph harvard {
  ...
  ctrl [label="Control Unit", shape=ellipse, fillcolor="#d6eef5"];
  alu  [label="ALU", shape=box, fillcolor="#fff3cd"];
  imem [label="Instruction\nMemory", shape=box, fillcolor="#e8f4f8"];
  dmem [label="Data\nMemory", shape=box, fillcolor="#d7f0c2"];
  io   [label="I/O", shape=box, fillcolor="#f9d9e2"];
...
digraph decision {
  ...
  inp  [label="Mathematical\nStatement", shape=box, fillcolor="#e8f4f8"];
  alg  [label="Algorithm?", shape=ellipse, fillcolor="#fff3cd"];
  yes  [label="TRUE", shape=box, fillcolor="#d7f0c2"];
  no   [label="FALSE", shape=box, fillcolor="#f9d9e2"];
...
```{dot}
//| fig-cap: "TM strategy for wcw: copy the left block, then compare the right block"
//| fig-width: 6.2
//| fig-height: 2.4
digraph tm_wcw {
  rankdir=LR;
  ...
  copy [label="q0\nCopy w before c\nto memory tape", fillcolor="#e8f4f8"];
  rewind [label="q1\nRewind memory", fillcolor="#d6eef5"];
  compare [label="q2\nCompare second w\nwith memory", fillcolor="#fff3cd"];
  acc [label="qF\nAccept", fillcolor="#d7f0c2"];

  copy -> rewind [label="read c"];
  rewind -> compare [label="memory at Z₀"];
  compare -> acc [label="both tapes end together"];
}
```
...
```{dot}
//| fig-cap: "Finite-control view of the TM for (ab)ⁿ"
//| fig-width: 4.8
//| fig-height: 2
digraph tm_abn {
  rankdir=LR;
  ...
  dead [label="dead", fillcolor="#f9d9e2"];
  start -> q0;
  q0 -> q1 [label="a"];
  q1 -> q0 [label="b"];
  q0 -> dead [label="b"];
...
```{dot}
//| fig-cap: "TM strategy for wcwᴿ: compare the second block against memory in reverse direction"
//| fig-width: 6.2
//| fig-height: 2.4
digraph tm_wcwr {
  rankdir=LR;
  ...
  copy [label="q0\nCopy w", fillcolor="#e8f4f8"];
  pivot [label="on c\nturn around", fillcolor="#d6eef5"];
  cmp [label="q1\nInput head → right\nMemory head → left", fillcolor="#fff3cd"];
  acc [label="qF", fillcolor="#d7f0c2"];

  copy -> pivot;
  pivot -> cmp;
  cmp -> acc [label="all matches"];
}
```
...
```{dot}
//| fig-cap: "TM memory layout for checking aⁿb²ⁿc³ⁿ"
//| fig-width: 6.2
//| fig-height: 2.1
digraph tm_ratio123 {
  rankdir=TB;
  ...
  input [shape=record, style=filled, fillcolor="#e8f4f8", label="Input: | a | a | ... | b | ... | c | ... |"];
  mem [shape=record, style=filled, fillcolor="#fff3cd", label="Memory: | Z₀ | B | B | ... | C | C | C | ... |"];
  note [shape=box, style="filled,rounded", fillcolor="#d7f0c2", label="For each a:\nwrite BBCCC"];

  input -> mem [label="encode counts"];
...
```{dot}
//| fig-cap: "TM palindrome strategy: compare the copied word against the original read backwards"
//| fig-width: 6
//| fig-height: 2.2
digraph tm_pal {
  rankdir=LR;
  ...
  copy [label="Copy w to memory", fillcolor="#e8f4f8"];
  rewind [label="Rewind memory", fillcolor="#d6eef5"];
  compare [label="Compare memory forward\nwith input backward", fillcolor="#fff3cd"];
  acc [label="Accept", fillcolor="#d7f0c2"];
...
```{dot}
//| fig-cap: "TM for {aⁿbⁿ} ∪ {aⁿb²ⁿ}: try one counting strategy, then reset and try the other"
//| fig-width: 6.2
//| fig-height: 2.3
digraph tm_union_counts {
  ...
  start [label="Start", fillcolor="#e8f4f8"];
  try1 [label="Check aⁿbⁿ", fillcolor="#d6eef5"];
  reset [label="Reset tapes / heads", fillcolor="#fff3cd"];
  try2 [label="Check aⁿb²ⁿ", fillcolor="#f9d9e2"];
  acc [label="Accept if either succeeds", fillcolor="#d7f0c2"];

  start -> try1;
  try1 -> acc [label="success"];
  try1 -> reset [label="fail"];
  reset -> try2;
  try2 -> acc [label="success"];
}
```
...
```{dot}
//| fig-cap: "Control phases of the TM trace for aⁿbⁿ"
//| fig-width: 5.5
//| fig-height: 2.1
digraph tm_trace_abn {
  ...
  q1 [label="q1\nwrite M", fillcolor="#d6eef5"];
  q2 [label="q2\nmatch b", fillcolor="#fff3cd"];
  qf [label="qF", shape=doublecircle, fillcolor="#d7f0c2"];
  q0 -> q1 [label="first a"];
  q1 -> q1 [label="a"];
  q1 -> q2 [label="first b"];
...
```{dot}
//| fig-cap: "Three-phase TM for aⁿbⁿcⁿ"
//| fig-width: 6
//| fig-height: 2.2
digraph tm_trace_abcn {
  ...
  node [fontname="Helvetica", fontsize=11, shape=circle, style=filled, color="#355c7d", penwidth=1.1];
  edge [fontname="Helvetica", fontsize=10, color="#355c7d"];
  q1 [label="q1\nmark a", fillcolor="#e8f4f8"];
  q2 [label="q2\nmatch b", fillcolor="#d6eef5"];
  q3 [label="q3\nmatch c", fillcolor="#fff3cd"];
  qf [label="qF", shape=doublecircle, fillcolor="#d7f0c2"];
  q1 -> q1 [label="a"];
  q1 -> q2 [label="b"];
  q2 -> q2 [label="b"];
  q2 -> q3 [label="c with Z₀"];
  q3 -> q3 [label="c with M"];
  q3 -> qf [label="blank"];
}
```
...
```{dot}
//| fig-cap: "Single-tape TM loop for aⁿbⁿ"
//| fig-width: 6
//| fig-height: 2.2
digraph tm_mark_pairs {
  ...
  finda [label="q0\nFind leftmost a\nand mark X", fillcolor="#e8f4f8"];
  findb [label="q1\nScan right for b\nand mark Y", fillcolor="#d6eef5"];
  back [label="q2\nReturn left", fillcolor="#fff3cd"];
  acc [label="qF", fillcolor="#d7f0c2"];

  finda -> findb -> back -> finda;
  finda -> acc [label="no a left"];
}
```
...
```{dot}
//| fig-cap: "Single-tape TM loop for aⁿbⁿcⁿ"
//| fig-width: 6.2
//| fig-height: 2.3
digraph tm_mark_triplets {
  ...
  a [label="q0\nmark a → X", fillcolor="#e8f4f8"];
  b [label="q1\nmark b → Y", fillcolor="#d6eef5"];
  c [label="q2\nmark c → Z", fillcolor="#fff3cd"];
  back [label="q3\nreturn left", fillcolor="#f9d9e2"];
  check [label="q4\nverify only Y,Z remain", fillcolor="#d7f0c2"];

  a -> b -> c -> back -> a;
  a -> check [label="no a left"];
}
```

...
a) A graph with 6 vertices arranged like an octahedron with crossing edges in the center

![](11_1.png)

b) A linear arrangement of 5 vertices on a bottom line with 5 vertices above them, connected by arching edges creating crossing loops
c) A graph with 8 vertices arranged in two columns of 4, with horizontal, vertical, and diagonal connections crossing each other

<details>
<summary>Click to see the solution</summary>

**Key Concept:** To show a graph is planar, we need to redraw it so that no edges cross. This often involves "pulling" some edges to the outside of the figure.

**(a) Octahedron-like graph:**

1.  **Identify the structure:** This graph has 6 vertices forming an octahedron-like structure.
2.  **Redraw:** Draw 4 vertices as a square, place one vertex in the center, and one vertex outside. Connect edges appropriately, curving the edge that would cross to go around the outside.
3.  The graph is planar because it can be redrawn without crossings.

**(b) Linear arrangement with arches:**

1.  **Identify the structure:** This is likely a bipartite graph with 5+5 vertices.
2.  **Redraw:** Rearrange vertices so that one set forms the boundary and connect to the inner set without crossings.
3.  The graph is planar.

**(c) Two columns of 4 vertices:**

1.  **Identify the structure:** This 8-vertex graph forms a grid-like pattern.
2.  **Redraw:** Recognize this as a planar graph (similar to a cube or ladder graph). Redraw as a "box within a box" structure.
3.  The result is a planar embedding.

**Answer:** All three graphs can be redrawn without edge crossings, proving they are planar.
...
- Left Graph: A 7-vertex graph with vertices arranged in a heptagon with an outer cycle and an inner star-like pattern
- Right Graph: A 7-vertex graph with dense connections crossing in the center

<details>
<summary>Click to see the solution</summary>

**Key Concept:** Use Kuratowski's Theorem—if a graph contains a subdivision of $K_5$ or $K_{3,3}$ as a subgraph, it is not planar.

**Left Graph:**
1.  **Analyze the structure:** Count vertices $v = 7$ and edges.
2.  **Check edge bound:** Calculate if $e \leq 3v - 6 = 3(7) - 6 = 15$.
3.  **Look for $K_5$ or $K_{3,3}$ subdivision:** If not found and edge bound is satisfied, attempt to draw without crossings.
4.  Determine planarity based on analysis.

**Right Graph:**
1.  **Analyze the structure:** This appears to be a more densely connected graph.
2.  **Search for forbidden subgraphs:** Look for a subdivision of $K_5$ or $K_{3,3}$.
3.  If found, the graph is **not planar**.

**Answer:** The planarity depends on whether each graph contains a subdivision of $K_5$ or $K_{3,3}$. Detailed analysis of the specific edge connections is required.
...
a) A "kite" shaped graph with 4 vertices (a, b, c, d) where b and d are connected by a horizontal edge
b) A graph with 5 vertices (a, b, c, d, e) consisting of a triangle and a path
c) The complete graph $K_6$

<details>
<summary>Click to see the solution</summary>

**Key Concept:** The chromatic number is the minimum number of colours needed for a proper colouring. Look for cliques (complete subgraphs) as they give lower bounds.

**(a) Kite graph:**

1.  **Analyze structure:** 4 vertices forming a diamond shape with vertices a (top), b (left), c (bottom), d (right), with edges a-b, a-d, b-c, c-d, and b-d.
2.  **Find cliques:** Triangle b-c-d forms a 3-clique, so $\chi(G) \geq 3$.
3.  **Attempt 3-colouring:**
    *   Colour b = 1, c = 2, d = 3
    *   Vertex a is adjacent to b and d, so a ≠ 1 and a ≠ 3. Use a = 2.
4.  **Result:** $\chi(G) = 3$

**(b) Triangle with path:**

1.  **Analyze structure:** Identify the triangle and how the path connects.
2.  **Find cliques:** If there's a triangle, $\chi(G) \geq 3$.
3.  **Attempt 3-colouring:** Colour the triangle with 3 colours, then extend to remaining vertices.
4.  **Result:** $\chi(G) = 3$ (assuming standard configuration)

**(c) Complete graph $K_6$:**

1.  **Property of complete graphs:** In $K_n$, every vertex is adjacent to every other vertex.
2.  **Consequence:** Each vertex must have a distinct colour.
3.  **Result:** $\chi(K_6) = 6$
...
2.  **Attempt with 3 colours:**
    ...
    *   Colour the outer vertices alternating: 1, 2, 1, 2, 1, 2, 1
    *   Wait — this fails because 7 is odd! We get: 1, 2, 1, 2, 1, 2, and the 7th must differ from both the 6th (colour 2) and the 1st (colour 1), so it needs colour 3.

...
In other words, $L$ consists of exactly those strings that, when processed by $M$, lead to an accepting state.
...
##### **4.1. Converting State Diagram to State Table** (Tutorial 4, Example 1)
...
##### **4.2. Completing an Incomplete FSA** (Tutorial 4)
...
##### **4.3. Complement of a Complete FSA** (Tutorial 4, Example 2)
...
##### **4.4. Complement of an Incomplete FSA** (Tutorial 4)
...
##### **4.5. Intersection of Two FSAs** (Tutorial 4, Example 3)
...
##### **4.6. Intersection of Complex FSAs** (Tutorial 4)
...
3.  **Identify reachable states from $(q_0, p_0)$:**
    *   $(q_0, p_0)$ — initial
    *   $(q_0, p_1)$ — from $(q_0, p_0)$ on 'a'
    *   $(q_1, p_0)$ — from $(q_0, p_0)$ on 'b'
    *   $(q_1, p_1)$ — from $(q_0, p_1)$ on 'b' is actually to $(q_1, p_0)$... Let me recalculate carefully.

    Actually, tracing carefully:
    *   Start: $(q_0, p_0)$
    *   Read 'a': go to $(q_0, p_1)$
    *   Read 'a' again: go to $(q_0, p_2)$
    *   From $(q_0, p_0)$, read 'b': go to $(q_1, p_0)$
    *   From $(q_1, p_0)$, read 'a': go to $(q_2, p_1)$
    *   Continue tracing...

    The reachable states turn out to be: $(q_0, p_0), (q_0, p_1), (q_1, p_0), (q_1, p_1), (q_2, p_2)$, and states involving $q_3$ and trap behavior.
4.  **Simplified automaton (after removing unreachable states):**
    The key accepting path is: $(q_0, p_0) \xrightarrow{a} (q_0, p_1) \xrightarrow{b}$ (no, this doesn't work)...

    Actually the correct path to $(q_2, p_2)$ is:
    *   Start at $(q_0, p_0)$
    *   Read 'a': go to $(q_0, p_1)$ (since $q_0 \xrightarrow{a} q_0$ and $p_0 \xrightarrow{a} p_1$)
    *   Read 'b': go to $(q_1, p_0)$ (since $q_0 \xrightarrow{b} q_1$ and $p_1 \xrightarrow{b} p_0$)

    Let me think more carefully about what paths lead to $(q_2, p_2)$...

    To reach $q_2$ in $M_1$: must go $q_0 \xrightarrow{b} q_1 \xrightarrow{a} q_2$
    To reach $p_2$ in $M_2$: must go $p_0 \xrightarrow{a} p_1 \xrightarrow{a} p_2$

    So we need to coordinate: start $(q_0, p_0)$, somehow get to $(q_2, p_2)$.
    Path: $(q_0, p_0) \xrightarrow{a} (q_0, p_1) \xrightarrow{a} (q_0, p_2) \xrightarrow{b} (q_1, p_2) \xrightarrow{a} (q_2, p_2)$ — but wait, from $p_2$, we loop on both symbols, so $p_2 \xrightarrow{a} p_2$ and $p_2 \xrightarrow{b} p_2$.

    So the correct path is: $(q_0, p_0) \xrightarrow{a} (q_0, p_1) \xrightarrow{a} (q_0, p_2)$ is wrong because $q_0 \xrightarrow{a} q_0$ not $q_1$.

    Let's trace "aa" (which should lead to $p_2$ in $M_2$):
    *   $(q_0, p_0) \xrightarrow{a} (q_0, p_1) \xrightarrow{a} (q_0, p_2)$

    And from $(q_0, p_2)$, to reach $q_2$, we need: $q_0 \xrightarrow{b} q_1 \xrightarrow{a} q_2$. And $p_2$ loops on everything.

    So: $(q_0, p_2) \xrightarrow{b} (q_1, p_2) \xrightarrow{a} (q_2, p_2)$ — accepting!
...
##### **4.7. Union of Two FSAs** (Tutorial 4, Example 4)

Using the same $M_1$ and $M_2$ from Example 4.5 (odd-length strings and all strings), construct $M_1 \cup M_2$.
...
##### **4.8. Difference of Two FSAs** (Tutorial 4, Examples 5-6)

**Example 4.8a:** Compute $M_1 \setminus M_2$ where $M_1$ accepts odd-length strings and $M_2$ accepts all strings (from previous examples).
...
**Example 4.8b:** Compute $M_2 \setminus M_1$ (all strings minus odd-length strings).
...
##### **4.9. Constructing an FST: Past Tense to Present Tense** (Lab 3, Exercise 1)
...
##### **4.10. Constructing an FST: Erasing Every Second 'a'** (Lab 3, Exercise 2)
...
##### **4.11. Building FSAs for Even 1's and Odd 0's** (Lab 3, Exercise Part 1, Problems 1-2)
...
##### **4.12. Combining FSAs: Union, Intersection, Difference** (Lab 3, Exercise Part 1, Problems 3-6)
...
##### **4.13. Complement of Incomplete FSA** (Lab 3, Exercise Part 2)
...
##### **4.14. FSAs for Divisibility by 2 and 3** (Lab 3, Exercise Part 3, Problems 1-2)
...
##### **4.15. Combining Divisibility FSAs** (Lab 3, Exercise Part 3, Problems 3-5)
...
##### **4.16. Complex Product Construction** (Lab 3, Exercise Part 4)
